...
public class CardExRV extends JFrame{
	CardLayout card;
	Container con;
	public CardExRV() {
		super("CardLayout 예제복습");
		card = new CardLayout();//카드 레이아웃 생성
		con = getContentPane();//프레임의 컨텐츠 영역을 만들어 처리
		setLayout(card);//기본레이아웃을 카드 레이아웃으로 변경
		JPanel[] pan = new JPanel[5]; // 패널 배열 생성
		Color[] color = {Color.red,Color.black,Color.cyan,Color.orange,Color.BLUE};
