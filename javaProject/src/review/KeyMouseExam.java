...
public class KeyMouseExam extends JApplet implements MouseListener,MouseMotionListener,KeyListener{
	...
	private boolean flag = false; //이미지 처음실행할땐 안보이게 처리하기 위해.
	
	@Override
	public void init() {
		JLabel j = new JLabel("!!!한훈근 변호사 시험 합격기원!!!");
		JLabel j1 = new JLabel("!!!모두가 응원합니다.!!!");
		JLabel j2 = new JLabel("feat.작은 누나의 댄스");
		add(j);add(j1);add(j2);
		j.setFont(new Font("굴림", Font.BOLD, 60));
		j1.setFont(new Font("굴림", Font.BOLD, 60));
		j2.setFont(new Font("굴림", Font.BOLD, 30));
		setLayout(new FlowLayout());
		setSize(1200, 1200);
		//이미지를 불러오는 코드
		img = Toolkit.getDefaultToolkit().getImage(getClass().getResource("luvu.gif"));
